// File: app/(menu)/WalletsPage/page.tsx
'use client';
...
const DEBUG_ENABLED = process.env.NEXT_PUBLIC_DEBUG_LOG_WALLETS === 'true';
const debugLog = createDebugLogger('WalletsPage', DEBUG_ENABLED, LOG_TIME);
...
export default function Page() {
  ...
  useEffect(() => {
    async function loadWallets() {
      try {
        const data = await getJson<WalletAccount[]>('/wallets.json');
        setWallets(data ?? []);
        debugLog.log?.(`[WalletsPage] Loaded ${data?.length ?? 0} wallets`);
      } catch (err) {
        debugLog.error?.('[WalletsPage] Failed to load wallets', err);
        setWallets([]);
      }
    }
    loadWallets();
