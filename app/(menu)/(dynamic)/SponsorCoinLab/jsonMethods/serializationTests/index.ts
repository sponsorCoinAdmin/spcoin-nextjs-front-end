...
async function loadSponsorAccounts(access: ReturnType<typeof createSpCoinLibraryAccess>) {
  const getMasterAccountList = (access.read as any).getMasterAccountList ?? access.read.getAccountList;
  if (typeof getMasterAccountList !== 'function' || typeof access.read.getAccountRecipientList !== 'function') {
    throw new Error(`Sponsor-list utilities require getMasterAccountList() and getAccountRecipientList() read methods.`);
  }
  const accountList = Array.from((await getMasterAccountList()) as unknown[]).map((value) => normalizeAddress(value));
...
async function loadAccountList(access: ReturnType<typeof createSpCoinLibraryAccess>) {
  ...
  return Array.from((await getMasterAccountList()) as unknown[]).map((value) => normalizeAddress(value));
}
