...
type Props = {
    ...
    selectElement: any,
    getSelectedListElement:  (listElement: TokenElement) => void,
}

function displayElementDetail (le: TokenElement) {
    alert("displayElementDetail\n" + JSON.stringify(le,null,2))
}

function WalletDataList({dataList, selectElement, getSelectedListElement} : Props) {
    // alert("dataList = " + JSON.stringify(dataList,null,2));

    const tList = dataList?.map((e: TokenElement, i: number) => (
