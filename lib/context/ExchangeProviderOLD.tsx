...
// 🔹 App-level account hook (ExchangeContext)
import { useAppAccount } from '@/lib/context/hooks/nestedHooks/useAccounts';
// 🔹 UI-level connected account (wallet.json / fallback)
import {
  useConnectedAccount as useUiConnectedAccount,
} from '@/lib/context/ConnectedAccountContext';
...
const ensurePanelNamesInMemory = (panels: PanelNode[]): PanelNode[] =>
...
function persistWithOptDiff(
  ...
  try {
    if (DEBUG_ENABLED) {
      ...
      const prevPanels = (prevCtx as any)?.settings
        ?.spCoinPanelTree as FlatPanelLite[] | undefined;
      const nextPanels = (nextCtx as any)?.settings
        ?.spCoinPanelTree as FlatPanelLite[] | undefined;
      ...
      const changeLine =
        changes.length === 0
          ...
          : changes
              .map(
                (c) =>
                  `${panelName(c.id)} ${String(c.before)} → ${String(
                    c.after,
                  )}`,
              )
              .join(', ');
...
// 🔹 App-level bootstrap to mirror UI connectedAccount → ExchangeContext.appAccount
const appBootstrapLog = createDebugLogger(
  'AppBootstrap',
...
function AppBootstrap() {
  const connectedAccount = useUiConnectedAccount(); // from ConnectedAccountContext
  const [appAccount, setAppAccount] = useAppAccount(); // from ExchangeContext

  useEffect(() => {
    const connectedAddr = connectedAccount?.address?.toLowerCase?.();
    const appAddr = appAccount?.address?.toLowerCase?.();

    appBootstrapLog.log?.('🧮 account mirror check', {
      connectedAddr,
      appAddr,
    });

    // No connected wallet: deliberately do NOT clear appAccount
    if (!connectedAddr) {
      appBootstrapLog.log?.(
    ...
    if (!appAddr) {
      appBootstrapLog.log?.('🔄 Seeding appAccount from connectedAccount', {
        connectedAddr,
      });
    ...
    if (connectedAddr !== appAddr) {
      appBootstrapLog.log?.('🔄 Updating appAccount to follow connectedAccount', {
        from: appAddr,
        to: connectedAddr,
      });
      setAppAccount(connectedAccount);
    ...
    appBootstrapLog.log?.(
      '✅ appAccount already matches connectedAccount — no-op',
      { connectedAddr, appAddr },
    );
...
export function ExchangeProvider({ children }: { children: React.ReactNode }) {
  ...
  const setExchangeContext = (
    ...
    setContextState((prev) => {
      ...
      try {
        ...
        (normalized as any).settings['spCoinPanelSchemaVersion'] =
        ...
        debugLog.warn?.(
          '⚠️ setExchangeContext persist path failed, falling back. Reason:',
          err,
        );
        try {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { panels: validated } = validateAndRepairPanels(
        repaired.map(ensurePanelName),
      );
  ...
  useEffect(() => {
    ...
    if (
      isConnected &&
      address &&
      !contextState.accounts?.connectedAccount?.address
    ) {
      setExchangeContext(
        (prev) => {
          const next = clone(prev);
...
function repairPanels(
  persisted:
    | Array<{ panel: number; name?: string; visible?: boolean }>
    | undefined,
): PanelNode[] {
  ...
  if (Array.isArray(persisted)) {
    for (const p of persisted) {
      ...
      if (
        !Number.isFinite(id) ||
        NON_PERSISTED_PANELS.has(id as SP_COIN_DISPLAY)
      )
        continue;
