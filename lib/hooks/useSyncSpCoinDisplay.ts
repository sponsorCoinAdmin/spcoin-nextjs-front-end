...
export const useSyncSpCoinDisplay = () => {
  ...
  useEffect(() => {
    ...
    if (isSellSp) nextDisplay = SP_COIN_DISPLAY.MANAGE_RECIPIENT_BUTTON;
    else if (isBuySp) nextDisplay = SP_COIN_DISPLAY.SELECT_RECIPIENT_BUTTON;

    console.debug('   isSellSp:', isSellSp, '→', SP_COIN_DISPLAY.MANAGE_RECIPIENT_BUTTON);
    console.debug('   isBuySp:', isBuySp, '→', SP_COIN_DISPLAY.SELECT_RECIPIENT_BUTTON);
