...
import infoIcon from '@/public/assets/miscellaneous/info.png';
...
type BaseListRowProps = {
  ...
  avatarClickAction?: 'select' | 'info';
  trailingControl?: 'select' | 'info';
  selectTitle?: string;
  className?: string;
...
function BaseListRow({
  ...
  onInfoContextMenu,
  avatarClickAction,
  trailingControl = 'select',
  selectTitle,
  className,
  titleClassName = 'font-semibold truncate text-[#5981F3]',
  subtitleClassName = 'text-sm truncate',
}: BaseListRowProps) {
  const selectLabel = selectTitle ?? 'Select';
  const metaLabel = `${subtitle || title} Meta Data`;
  const resolvedAvatarClickAction =
    avatarClickAction ?? (onInfoClick ? 'info' : 'select');
    ...
    >
      <div className="flex items-center gap-3">
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            if (resolvedAvatarClickAction === 'select') {
              onAvatarClick();
              return;
            }
            onInfoClick?.();
          ...
          title={resolvedAvatarClickAction === 'select' ? selectLabel : metaLabel}
        >
          <img
      ...
      <button
        ...
        aria-label={trailingControl === 'info' ? `${title} metadata` : 'Select'}
        title={trailingControl === 'info' ? metaLabel : selectLabel}
      >
