...
type TokenListItemProps = {
  ...
  address: string;
  logoURL?: string;
  token?: Partial<TokenContract> & Record<string, unknown>;
...
const TokenListItem = React.memo(function TokenListItem({
  ...
  const previewToken = (): TokenContract => ({
    ...
    logoURL: logoURL || String(token?.logoURL ?? ''),
    balance: (token?.balance as bigint | undefined) ?? 0n,
  });

  const openPreview = () => {
    const tokenContract = previewToken();
    setPreviewTokenSource(buyMode ? 'BUY' : sellMode ? 'SELL' : null);
    setPreviewTokenContract(tokenContract);
    openPanel(
  ...
  return (
    <BaseListRow
      ...
      subtitleClassName="text-sm truncate !text-[#5981F3]"
    />
  );
});
