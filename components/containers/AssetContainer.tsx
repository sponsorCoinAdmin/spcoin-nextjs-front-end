...
const priceInputContainer = ({
  ...
  // ✅ Get exchangeContext using Hook
  ...
  useEffect(() => {
    if (activeContract) activeContract.balance = balanceInWei || 0n;
  }, [balanceInWei]);
  ...
  const setTextInputValue = (stringValue: string) => {
    ...
    setTransactionType(priceInputContainerType === CONTAINER_TYPE.INPUT_SELL_PRICE ? TRANSACTION_TYPE.SELL_EXACT_OUT : TRANSACTION_TYPE.BUY_EXACT_IN);
  };
