...
const ELEMENT_DETAILS =
  "This container allows for the entry selection of a valid recipient address.\n" +
...
export default function Dialog({ showDialog, setShowDialog, callBackWallet }: Props) {
  ...
  const setWalletDetails = async (walletAddr: any) => {
    try {
      ...
      alert("ERROR: setWalletDetails " + e.message);
    }
    return false;
  };
  ...
  const getSelectedListElement = (walletAccount: WalletAccount | undefined) => {
    ...
    if (walletAccount?.address === exchangeContext.agentAccount?.address) {
      alert("Recipient cannot be the same as Agent (" + exchangeContext.agentAccount.symbol + ")");
      console.log("Recipient cannot be the same as Agent (" + exchangeContext.agentAccount.symbol + ")");
      return false;
    }

    walletAccount.avatar = getAddressAvatar(walletAccount.address as Address, FEED_TYPE.RECIPIENT_WALLETS);

    // ✅ Update Exchange Context with new recipient wallet
    setExchangeContext({
      ...exchangeContext,
        ...
        <div className={styles.modalElementSelect}>
          <div className={styles.leftH}>
            ...
            <input
              ...
              onChange={setRecipientInputField}
