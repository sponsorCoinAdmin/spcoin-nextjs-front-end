...
const ExchangeButton = ({ isLoadingPrice, errorMessage, setErrorMessage, setResetAmounts }: Props) => {
  ...
  const transactionType: string =
    tradeData.transactionType === TRANSACTION_TYPE.SELL_EXACT_OUT ? "EXACT OUT " : "EXACT IN ";
  ...
  const getSwapType = () => {
    ...
    if (isActiveAccountAddress(sellTokenContract?.address))
      if (isWrappedNetworkAddress(buyTokenContract?.address)) return "SWAP WRAP ( ETH -> WETH )";
      else return transactionType + `( WRAP ETH -> WETH ) -> ${buyTokenContract?.symbol}`;
    else if (isActiveAccountAddress(buyTokenContract?.address))
      if (isWrappedNetworkAddress(sellTokenContract?.address)) return "SWAP UN-WRAP\n( WETH -> ETH )";
      else return transactionType + `${sellTokenContract?.symbol} -> ( WETH -> ETH )`;
  ...
  const tokensRequired = (): boolean => sellTokenRequired() && buyTokenRequired();
  const sellTokenRequired = (): boolean => !tradeData.sellTokenContract;
  const buyTokenRequired = (): boolean => !tradeData.buyTokenContract;
  const amountRequired = (): boolean => tradeData.sellAmount === 0n && tradeData.buyAmount === 0n;
